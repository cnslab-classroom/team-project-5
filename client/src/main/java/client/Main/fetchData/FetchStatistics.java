...
public class FetchStatistics {
  ...
  private static void parseStatisticsJson(String json, StatisticsData statisticsData) {
    try {
      // "statics" 배열 파싱
      if (json.contains("\"statics\":[")) {
        String staticsSection = json.split("\"statics\":\\[")[1].split("\\]")[0];
        if (!staticsSection.trim().isEmpty()) {
          String[] staticsArray = staticsSection.split("\\},\\{");
          for (String staticEntry : staticsArray) {
            staticEntry = staticEntry.replace("{", "").replace("}", "");
            String date = extractValue(staticEntry, "date");
            double achievement = Double.parseDouble(extractValue(staticEntry, "achievement"));
      ...
      // "daily_statics" 배열 파싱
      if (json.contains("\"daily_statics\":[")) {
        String dailySection = json.split("\"daily_statics\":\\[")[1].split("\\]")[0];
        if (!dailySection.trim().isEmpty()) {
          String[] dailyArray = dailySection.split("\\},\\{");
          for (String dailyEntry : dailyArray) {
            dailyEntry = dailyEntry.replace("{", "").replace("}", "");
            String goalText = extractValue(dailyEntry, "goal_text");
            double dateAchievement = Double.parseDouble(extractValue(dailyEntry, "goal_date_achievement"));
            ...
            statisticsData.getDailyStatics()
                .add(new DailyStatistics(goalText, dateAchievement, wholeDate, goalPercent));
          }
        }
      }
